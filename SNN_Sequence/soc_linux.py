...
#rom soc_snn.ranc_3x2 import ranc_3x2
#from soc_snn.packetload import packetload

#from ranc_1x1.edabk_snn import edabk_snn
#from snn_3x2.SNN_3x2 import snn_3x2
# from clone.SNN_3x2 import snn_3x2
from ranc_sequence.SNN_3x2 import snn_3x2
...
def SoCLinux(soc_cls, **kwargs):
    class _SoCLinux(soc_cls):
        ...
        def integrated_snn3x2(self):
            self.clock_domains.cd_snn_clk = ClockDomain()
            self.crg.pll.create_clkout(self.cd_snn_clk, 100e6)
            snn_clk = ClockSignal("snn_clk")
            self.submodules.snn_3x2 = snn_3x2(self.platform)
            self.add_csr("snn_3x2")
            self.comb += self.snn_3x2.clk.eq(snn_clk)
